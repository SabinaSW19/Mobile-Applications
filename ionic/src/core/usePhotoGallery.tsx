...
export function usePhotoGallery() {
    ...
    const savePicture = async (photo: CameraPhoto, fileName: string): Promise<Photograph> => {
    ...
    useEffect(() => {
        const loadSaved = async () => {
            ...
            const photos = (photosString ? JSON.parse(photosString) : []) as Photograph[];
            for (let photo of photos) {
                try {
                    ...
                    photo.photo = `data:image/jpeg;base64,${file.data}`;
                } catch (error) {
                    setPhotos([]);
                    return;
                }
